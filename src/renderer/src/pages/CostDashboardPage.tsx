...
import { fetchCost, type FilterType } from "@/apis/bills";
...
function isGranularityType(val: string): val is GranularityType {
  return GranularityValues.includes(val as GranularityType);
}

type ParsedGroup = [string, string];
type ParsedCost = {
  name: string;
} & Record<string, string>;

function parseGroups(groups: Group[]): Record<string, string> {
  return Object.fromEntries(
    groups.map((group) => {
      const tuple: ParsedGroup = [
        group.Keys[0],
...
function parseFetchCostResult(res: ResultByTime[]): ParsedCost[] {
  return res.map((elem) => {
    return {
      /*
...
export default function CostDashboardPage() {
  ...
  const [data, setData] = useState<ParsedCost[]>();
  useEffect(() => {
    (async () => {
  ...
  const updateGranularity = (newGranularity: GranularityType) => {
    const newFilters = { ...filters, granularity: newGranularity };
    setFilters(newFilters);
  };

  const updateRecordTypes = (newRecordTypes: string) => {
