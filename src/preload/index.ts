...
const api: CustomAPI = {
  s3: {
    ...
    deleteObject: (bucketName: string, objectName: string) =>
      ipcRenderer.invoke("s3:delete-object", { bucketName, objectName }),
    getObject: (bucketName: string, objectName: string) =>
      ipcRenderer.invoke("s3:get-object", { bucketName, objectName })
  },
  ec2: {
  ...
  services: {
    ...
    onCreateLog: (callback) =>
      createInternalListener<StreamData>("services-internal:create-log", callback),
    onUpdateLog: (id, callback) =>
      createInternalListener<StreamData>(`services-internal:${id}-update-log`, callback),
    onDeleteLog: (id, callback) =>
      createInternalListener<StreamData>(`services-internal:${id}-delete-log`, callback)
  },
  ...
  deploys: {
    ...
    streamBuild: (buildId) => ipcRenderer.invoke("deploys:stream-build", { buildId }),

    onStatusLog: (executionId, callback) =>
      createInternalListener<PipelineLogData>(
        `deploys-internal:execution-${executionId}`,
        callback
      ),
