...
} from "@/services/deploymentService/deployment.schema";
...
export type CustomAPI = {
  s3: {
    ...
    uploadObject: (
      ...
      buffer: ArrayBuffer,
      mimeType: string
    ) => Promise<IPCMessageResponse>;
    deleteObject: (bucketName: string, objectName: string) => Promise<IPCMessageResponse>;
    getObject: (
      bucketName: string,
      objectName: string
    ) => Promise<IPCDataResponse<{ file: Uint8Array<ArrayBufferLike>; type: string | undefined }>>;
  };
  ec2: {
  ...
  bill: {
    getCost: (filter: {
      ...
      recordTypes: string;
    }) => Promise<IPCDataResponse<ResultByTime[]>>;
  };
  services: {
  ...
  onboarding: {
    start: (credentials: {
      ...
      secretKey: string;
      region: string;
    }) => Promise<IPCSuccessResponse<void>>;
