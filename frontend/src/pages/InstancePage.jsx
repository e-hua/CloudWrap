...
function InstanceImageGroup({ newInstanceImage, setNewInstanceImage }) {
  return (
...
function InstanceCostPanel({ instanceType, instanceImage }) {
  const hourly_cost =
    INSTANCE_TYPE_MAPPING[instanceType]?.[
      `${instanceImage.toLowerCase()}_price`
    ];
    ...
    <div className="w-40">
      <p className="text-text-secondary font-mono text-wrap text-sm">
        ...
        <span className="text-accent">
          {isNaN(monthly_cost) ? " Not available" : ` $${monthly_cost}`}
        </span>
      </p>
    </div>
  );
...
export default function InstancePage() {
  ...
  const onCreateNewInstance = async function () {
    try {
      console.log({ newInstanceName, newInstanceImage, newInstanceType });
