    ...
    <>
      ...
      <CardEntry entryName="Instance state">
        <div className="flex flex-row items-center gap-2">
          {(STATE_MAPPING[instanceState] || STATE_MAPPING["unknown"]).icon}
          <p
            className={clsx(
              "text-xs",
              (STATE_MAPPING[instanceState] || STATE_MAPPING["unknown"])
                .className
            )}
          >
            {instanceState}
      ...
      <CardEntry entryName="Launch time">
        <EntryText>{creationDate.substring(0, 10)}</EntryText>
      </CardEntry>
